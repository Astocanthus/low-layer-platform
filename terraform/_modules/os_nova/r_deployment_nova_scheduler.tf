...
# =============================================================================
# NOVA SCHEDULER DEPLOYMENT
# =============================================================================
# This resource provisions a Kubernetes Deployment that runs the Nova Scheduler
# service. The scheduler service determines which compute host will run a newly
# requested virtual machine instance based on available resources, placement
# constraints, and scheduling policies. It uses RPC-based health monitoring for
...
resource "kubernetes_deployment_v1" "nova_scheduler" {
  metadata {
    name      = "nova-scheduler"
  ...
  spec {
    ...
    template {
      ...
      spec {
        service_account_name             = "nova-scheduler"
        ...
        # -----------------------------------------------------------------------
        # MAIN CONTAINER - NOVA SCHEDULER
        # -----------------------------------------------------------------------
        # Runs the Nova Scheduler service.
        ...
        container {
          ...
          # HEALTH PROBES
          # -------------------------------------------------------------------
          # RPC-based liveness and readiness probes for scheduler service health.
          ...
          volume_mount {
            name       = "nova-bin"
            mount_path = "/tmp/nova-scheduler.sh"
            sub_path   = "nova-scheduler.sh"
            read_only  = true
          }
