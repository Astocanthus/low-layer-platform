...
logger = logging.getLogger('OpenStack-Terraform Keystone Endpoint management')
...
if "OPENSTACK_CONFIG_FILE" in os.environ:
    ...
    try:
        ...
        config.read(os_conf)
        logger.info("Trying to load db config from {0}:{1}".format(os_conf_section, os_conf_key))
...
try:
    with user_engine.connect() as connection:
        endpoints = connection.execute(
            text("select interface, url from endpoint where service_id = "" (select id from service where service.type = 'identity')")
        ).fetchall()
